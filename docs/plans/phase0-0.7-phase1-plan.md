...
| 07 | **Ground cover (T6)** | Canopy → forest mask and tree height; WorldCover classes; **species table from BIGMAP or LANDFIRE (TR3)**; OSM water and developed land rasterized at 1 m with anti-aliasing; soft class weights; terrain-aware boundaries; control/splat maps; cover-map overlay | Golden hashes stable. The forest-fraction check against WorldCover works. Nothing blocky at close range (checked in 08) | 1 wk |
...
| 09 | **Forest at scale (T7)** | The remaining Phase 1 species (Pacific silver fir, Douglas-fir, lodgepole pine, krummholz) and the `species-map`; deterministic Burst Poisson sampling per 64 m tile; species drawn from cell weights; our own GPU instancing and impostor baker (TR2) | About 650,000 trees on Crystal Mountain. Tile hashes are identical across runs. The forest fits within the frame budget in 15 | 1 wk |
