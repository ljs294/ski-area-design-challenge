...
Completed/current task: P1-A — renderer-independent session ownership and failure-isolated committed notifications.
Next task: P1-B — recoverable fork storage and atomic design/terrain publication.
...
Current entry points: the standalone P0 prototype remains under `prototypes/three-terrain/`. P1-A landed in `src/app/session/designSession.ts`, `terrainDocument.ts`, `topologyDocument.ts`, `committedDocumentTransaction.ts`, and the current `MapView.tsx` composition.
