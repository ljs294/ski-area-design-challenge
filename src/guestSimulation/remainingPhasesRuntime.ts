...
import type { FacilityContract } from './facilities.ts';
import { defaultBaseFacilities } from './defaultFacilities';
...
function defaultFacilities(network: GuestSimulationNetwork, startTick: number, endTick: number): readonly FacilityContract[] {
  return defaultBaseFacilities(network.portalConnections[0]?.nodeId ?? network.nodes[0]!.id, startTick, endTick);
}
