...
/** Analytical intersection with the package sampler. The interval step is tied
 * to source-cell size and bisection resolves the displayed surface; it never
 * scans rendered triangles and therefore remains invariant under display LOD. */
export function intersectTerrainRay(ray: Ray, record: TerrainRecord,
  ...
  const sourceCell = Math.min(frame.widthM, frame.depthM)
    / Math.max(2, record.sampleGridSize - 1);
  ...
  const residual = (distance: number): number | null => {
    ...
    const elevation = sampleTerrainElevation(record, lng, lat);
  ...
  for (let t = Math.min(exit, priorT + dt); t <= exit + 1e-6; t = Math.min(exit, t + dt)) {
    ...
    if (prior != null && value != null && ((prior >= 0 && value <= 0) || (prior <= 0 && value >= 0))) {
      ...
      const elevation = sampleTerrainElevation(record, ...lngLat);
      if (elevation == null) return null;
      return { point: [x, elevation - frame.centerElevationM, z], lngLat, distance };
    }
    if (t >= exit) break;
    priorT = t;
    prior = value;
